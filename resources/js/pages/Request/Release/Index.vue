...
// ── FORM ──────────────────────────────────────────────
const form = reactive<{ details: any[] }>({ details: [] });

const updateFormFromRequest = () => {
    form.details = request.value.details.map((detail: any) => ({
        id: detail.id,
...
const hasInvalidQuantities = computed(() => Object.values(validationErrors).some((v) => v === true));
...
onMounted(() => {
    ...
    if (!window.STPadServerLib) {
        ...
        script.onload = () => {
            ...
            console.log(
                'API methods:',
                window.STPadServerLib?.STPadServerLibApi ? Object.keys(window.STPadServerLib.STPadServerLibApi) : 'No API found',
            );
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="space-y-4 p-4">
            <div v-if="loading">
                ...
                <Skeleton class="mb-4 h-32 w-full" />
